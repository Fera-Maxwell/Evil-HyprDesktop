...
local tags = { "1", "2", "3", "4" } -- Example of nerd font icon tags

-- Font for the status bar (use "fc-list" to see available fonts)
local bar_font = "Monocraft Nerd Font:style=Bold:size=12"
...
oxwm.rule.add({ instance = "prismlauncher", floating = true })
oxwm.rule.add({ instance = "feh", floating = true })
...
-- Unoccupied tags
oxwm.bar.set_scheme_normal(colors.fg, colors.bg, "#7777ee")
-- Occupied tags
...
oxwm.key.bind({ modkey }, "R", oxwm.spawn({ "rofi -show drun" }))
...
oxwm.key.bind({ modkey, "Shift" }, "Up", oxwm.spawn("brightnessctl s 5%+"))
oxwm.key.bind({ modkey, "Shift" }, "Down", oxwm.spawn("brightnessctl s 5%-"))
oxwm.key.bind({ modkey, "Shift" }, "Left", oxwm.spawn("wpctl set-volume @DEFAULT_AUDIO_SINK@ -5%"))
oxwm.key.bind({ modkey, "Shift" }, "Right", oxwm.spawn("wpctl set-volume @DEFAULT_AUDIO_SINK@ +5%"))
...
oxwm.key.chord({
	{ { modkey }, "P" },
	{ {},         "1" }
}, oxwm.spawn("systemctl poweroff"))
oxwm.key.chord({
	{ { modkey }, "P" },
	{ {},         "2" }
}, oxwm.spawn("systemctl reboot"))
oxwm.key.chord({
	{ { modkey }, "P" },
	{ {},         "3" }
}, oxwm.spawn("systemctl suspend"))
oxwm.key.chord({
	{ { modkey }, "P" },
	{ {},         "4" }
}, oxwm.spawn("systemctl hibernate"))
...
oxwm.autostart("feh --bg-scale ~/Pictures/backgrounds/background1.jpg")
oxwm.autostart("dunst")
oxwm.autostart("nm-applet")
oxwm.autostart("xinput set-prop 'SYNA32AA:00 06CB:CE17 Touchpad' 'libinput Natural Scrolling Enabled' 1; xinput set-prop 'SYNA32AA:00 06CB:CE17 Touchpad' 'libinput Tapping Enabled' 1; xset r rate 300 30; setxkbmap -layout us,tr -option grp:shifts_toggle")
oxwm.autostart("bash -c 'while true; do setxkbmap -option ctrl:nocaps; sleep 5; done'")
